...
/**
 * A session in the clustered client proxy handles the interaction between the broker and all the clients in the same cluster; aka all the clients with the same
 * client ID. The {@link ConnectMessage} from the first client is used to connect to the broker. The {@link ConnectMessage}s from subsequent clients must be
 * exactly the same or the connection will be rejected. All implementations of this API must be thread safe.
 */
