...
public interface MqttClient {
	 ...
	 * <ol>
	 * <li>Establish a TCP connection to the broker</li>
	 ...
	 */
	ConnectReturnCode connect(String clientId, boolean cleanSession, int keepAliveSeconds) throws MqttTimeoutException, InterruptedException;
	 ...
	 *             Thrown when the {@link SynchronousMqttClient} implementation is used and the calling thread is {@link Thread#interrupt() interrupted}.
	 */
	void disconnect() throws MqttTimeoutException, InterruptedException;
	 ...
	 *             Thrown when the {@link SynchronousMqttClient} implementation is used and the calling thread is {@link Thread#interrupt() interrupted}.
	 */
	Subscription[] subscribe(Subscription[] subscriptions) throws MqttTimeoutException, InterruptedException;
	 ...
	 */
	List<Subscription> subscribe(List<Subscription> subscriptions) throws MqttTimeoutException, InterruptedException;
	 ...
	 *             Thrown when the {@link SynchronousMqttClient} implementation is used and the calling thread is {@link Thread#interrupt() interrupted}.
	 */
	void unsubscribe(String[] topics) throws MqttTimeoutException, InterruptedException;

	/**
	 * Unsubscribes from topics. This is the same as {@link #unsubscribe(String[])} except it uses {@link List lists} instead of arrays.
	 */
	void unsubscribe(List<String> topics) throws MqttTimeoutException, InterruptedException;
	 ...
	 */
	void publish(String topicName, QoS qos, byte[] payload) throws MqttTimeoutException, InterruptedException;
	 ...
	 * @see MqttClient#publish(String, QoS, byte[], boolean)
	 */
	void publish(String topicName, QoS qos, String payload, boolean retain) throws MqttTimeoutException, InterruptedException;

	/**
	 * Creates a new client to the same broker this client uses. The new client will also share the same {@link Executor} and {@link PublishListener} or
	 * {@link AsyncClientListener}. The new client will use the same implementation this client does ({@link SynchronousMqttClient synchronous} or
	 * {@link AsyncMqttClient asynchronous}). When <code>connect</code> is called on the new client a new TCP connection will be established to the broker. This
	 * method does not block regardless of the implementation used.
	 * 
	 * @return A new {@link MqttClient}
	 */
